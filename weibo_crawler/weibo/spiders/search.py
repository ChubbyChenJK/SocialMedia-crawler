...
class SearchSpider(scrapy.Spider):
    ...
    def parse_weibo(self, response):
        ...
        for sel in response.xpath("//div[@class='card-wrap']"):
            ...
            if info:
                ...
                if self.settings.get('DOWNLOAD_NEED', 0) == 1:
                    if pics:
                        self.download_media(pics, 'image', weibo['id'], keyword)
                    if video_url:
                        self.download_media([video_url], 'video', weibo['id'], keyword)
    ...
    def download_media(self, media_urls, media_type, post_id, keyword):
        """下载媒体资源到本地"""
        settings = self.settings
        base_path = f'./结果文件/{keyword}/images' if media_type == 'image' else f'./结果文件/{keyword}/videos'
        ...
        for idx, url in enumerate(media_urls):
            try:
                ...
                if response.status_code == 200:
                    ...
                    file_path = os.path.join(base_path, f"{post_id}_{idx + 1}{ext}")
                    with open(file_path, 'wb') as f:
                        for chunk in response.iter_content(1024):
                            f.write(chunk)
            except Exception as e:
                self.logger.warning(f"下载失败: {url}, 错误信息: {e}")
