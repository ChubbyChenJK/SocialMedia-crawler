...
# 提取 cookie 的前两个字段
cookie_parts = settings["cookie"].split(";")
cookie = ";".join(cookie_parts[:2]).strip() + ";"  # 保留前两项 auth_token=xxx; ct0=xxx;

# 构造 _filter 字符串
keywords = settings.get("keyword", "")
time_range = settings.get("time_range", "")
keyword_part = " ".join([k.strip() for k in keywords.split(",") if k.strip()])
start_date, end_date = time_range.split(":")
_filter = f"{keyword_part} until:{end_date} since:{start_date}"
...
down_count = settings.get("down_count", 50)
...
text_down = settings.get("text_down", False)
# 开启后变为文本下载模式，会消耗大量API次数
# 开启文本下载时 不要包含 filter:links

##########配置区域##########

max_concurrent_requests = 2     #最大并发数量，默认为2，遇到多次下载失败时适当降低，下载速度过慢则适当升高
